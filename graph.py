...
# This import trick makes it work across versions.
try:
    # Older versions
    from langgraph.checkpoint import MemorySaver as InMemorySaver  # type: ignore
except Exception:
    try:
        # Some builds expose InMemorySaver directly
...
# The Annotated[..., operator.add] bit tells LangGraph how to MERGE updates:
...
# Here: we take the last incoming message and append an "echo: ..." response.
def echo_node(state: EchoState) -> dict:
    last = state["messages"][-1] if state["messages"] else ""
    response = f"echo: {last}"
    # Return a *partial* update. Because of `operator.add`, this will be appended.
...
# - Add an edge to END (since this is a one-node graph)
...
# In dev, in-memory is perfect: zero setup, fast. (For prod you’ll use Postgres, etc.)
checkpointer = InMemorySaver()
app = graph.compile(checkpointer=checkpointer)


# ---- 5) Demo: run it from the CLI ----
# We'll call the graph twice using the SAME thread_id to show that state persists.
